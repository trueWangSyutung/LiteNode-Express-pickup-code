...
class PortsActivity : ComponentActivity() {
    var formats  = mutableStateListOf<PostStation>()
    var formatsNum = mutableStateOf(0)
    fun loadCodeFormat(){
        thread {
            val database: CodeDatabase =
                CodeDatabase.getDatabase(this@PortsActivity)
            val portsDao = database.portsDao()
            formatsNum.value = portsDao.getPostsCount()
            formats.clear()
            val list = portsDao.getAll()
            runOnUiThread {
                formats.addAll(list)
            }
        }
        ...
        Column(
            ...
            AnimatedVisibility(visible = isMore.value) {
                Row(
                    ...
                    Button(onClick = {
                        if (format.id==0L){
                            Toast.makeText(this@PortsActivity, "无法删除", Toast.LENGTH_SHORT).show()
                        }else{
                            if (showDialog.value) {
            ...
            LiteNoteTheme {
                Scaffold(modifier = Modifier.fillMaxSize(),
                    floatingActionButton = {
                        IconButton(
                            onClick = {
                                val intent = Intent(this@PortsActivity,
                                    PortsAddActivity::class.java)
                    ...
                    var fontColor = getDarkModeTextColor(this@PortsActivity)
                    MyDialog(
                        onDismissRequest = {
                            if (showDialog.value) {
                                showDialog.value = false
                                ...
                                Row(
                                    ...
                                    Button(onClick = {
                                        ...
                                        thread {
                                            ...
                                            formatDao.deletePost(deleteID.value)
                                            runOnUiThread {
                                                loadCodeFormat()
                                                Toast.makeText(this@PortsActivity, "删除成功", Toast.LENGTH_SHORT).show()
                                            }
